...
class BaseModel(object):
    ''' Base class for embedding-based relational learning models that use 
        ...
        createArguments self.train_step, and can optionally createArguments self.post_step for 
        post-processing.
    ...
    def __init__(self, embedding_size, maxnorm=1.0,
                 ...
                 opt=None, entity_coocurrence_matrix=None, smoothing_weight=0.0):
        ...
        with self.graph.as_default():
            self.head_input = tf.placeholder(tf.int32, shape=[batch_pos_cnt * 2])
            self.rel_input = tf.placeholder(tf.int32, shape=[batch_pos_cnt * 2])
            self.tail_input = tf.placeholder(tf.int32, shape=[batch_pos_cnt * 2])
            self.target = tf.placeholder(tf.float32, shape=[batch_pos_cnt * 2])
            if entity_coocurrence_matrix is not None and smoothing_weight > 0.0:
                self.do_smoothing = True
                self.entity_coocurrence_matrix = tf.constant(entity_coocurrence_matrix, dtype=tf.float32)
                # self.entity_coocurrence_matrix = entity_coocurrence_matrix
                self.smoothing_weight = smoothing_weight

    def _relation_smoothing_penalty(self, entity_embeddings):
        # bad
        #penalties = []
        #for i, e1 in enumerate(tf.unpack(entity_embeddings)):
        #    for j, e2 in enumerate(tf.unpack(entity_embeddings)):
        #        # if i < j:
        #            # loss += self.smoothing_penalty * self.entity_coocurrence_matrix[i, j] * tf.reduce_sum(tf.abs(tf.sub(e1, e2)))
        #        penalties.append(self.entity_coocurrence_matrix[i, j] *
        #                         # tf.reduce_sum(tf.abs(tf.sub(e1, e2))))   # l1 distance
        #                         tf.sqrt(tf.nn.l2_loss(tf.sub(e1, e2))))   # l2 distance
        #return self.smoothing_weight * tf.reduce_sum(penalties)

        # good
        e1 = tf.expand_dims(entity_embeddings, 0)
        e2 = tf.expand_dims(entity_embeddings, 1)
        diff = tf.sqrt(tf.maximum(self.EPS, tf.reduce_sum(tf.square((tf.sub(e1, e2))), reduction_indices=2)))
        diff = tf.reshape(diff, self.entity_coocurrence_matrix.get_shape())
        print("done building smoothing penalty")
        return self.smoothing_weight * tf.reduce_sum(self.entity_coocurrence_matrix * diff)
    ...
    def _norm_constraint_op(self, var_matrix, row_indices, maxnorm):     
        ...
        if isinstance(self.opt, tf.train.AdamOptimizer) or row_indices is None:
    ...
    def fit(self, train_triples, step_callback=None, initial_entity_embeddings=None, entity_cooccurence=None):
        ...
        Args:
            ...
            initial_entity_embeddings: seed entity embeddings to use instead of random
        '''
        if self.sess is not None:
            self.sess.close()
        self.sess = tf.Session(graph=self.graph)
        with self.graph.as_default():
            self._create_model(train_triples, initial_entity_embeddings)
        ...
        for i in range(self.max_iter):
            ...
            if self.smoothing_penalty is not None:
                fetches.append(self.smoothing_penalty)
            returned = self.sess.run(fetches, feed_dict)
            if i % 5000 == 0:
                print('loss: %s. smoothing penalty: %s' % (returned[1], returned[2] if len(returned) > 2 else 'n/a'))
            if self.post_step is not None:
                self.sess.run(self.post_step, feed_dict)
            # if model_file is not None and i > 0 and i % 1000 == 0:
            #     print('Saving model to %s' % model_file)
            #     self.saver.save(self.sess, model_file, global_step=(i / 1000))
    ...
    def predict(self, triples):
        ...
        '''
        batch_size = self.batch_pos_cnt * 2
        if len(triples) > batch_size:
            return self.predict_batches(triples)
        if len(triples) < batch_size:
            triples = np.concatenate((triples, np.zeros((batch_size - len(triples), triples.shape[1]), dtype=np.int32)),
                                     axis=0)
        feed_dict = self.create_feed_dict(triples, training=False)
        return self.sess.run(self.output, feed_dict=feed_dict)

    def predict_batches(self, triples):
        length = len(triples)
        batches = []
        batch_size = self.batch_pos_cnt*2
        for i in xrange(length % (batch_size)):
            batches.append(self.predict(triples[i*batch_size: batch_size]))

        return np.concatenate(tuple(batches), axis=0)[:length]

    def load(self, model_dir, train_triples):
        self.sess = tf.Session(graph=self.graph)
        with self.graph.as_default():
...
class TransE(BaseModel):
    ...
    def __init__(self, embedding_size, batch_pos_cnt=100, 
                 ...
                 margin=1.0, opt=None, entity_coocurrence_matrix=None, smoothing_weight=0.0):
    ...
    def _create_model(self, train_triples, initial_entity_embeddings=None):
        # Count unique items to determine embedding matrix sizes
        entity_cnt = len(set(train_triples[:,0]).union(train_triples[:,2]))
        rel_cnt = len(set(train_triples[:,1]))
        init_sd = 1.0 / np.sqrt(self.embedding_size)
        ...
        entity_init = tf.truncated_normal(entity_var_shape, stddev=init_sd) \
            if initial_entity_embeddings is None else initial_entity_embeddings
        ...
        if self.dist == 'manhattan':
            ...
            tail_norm = tf.nn.l2_normalize(tail_embed, dim=1)
            head_norm = tf.nn.l2_normalize(head_embed + rel_embed, dim=1)
            raw_output = tf.matmul(tail_norm, head_norm, transpose_b=True)
        else:
            raise Exception('Unknown distance type: %s' % self.dist)
        # Model output
        self.output, self.loss = ranking_margin_objective(raw_output, self.margin)
        print("done building loss...")
        if self.do_smoothing:
            self.smoothing_penalty = self._relation_smoothing_penalty(self.entity_embedding_vars)
            # Optimization with postprocessing to limit embedding vars to L2 ball
            self.train_step = self.opt.minimize(self.loss + self.smoothing_penalty)
        else:
            self.train_step = self.opt.minimize(self.loss)
        unique_ent_indices = tf.unique(tf.concat(0, [self.head_input, self.tail_input]))[0]
...
class TransR(BaseModel):
  ...
  def __init__(self, embedding_size, batch_pos_cnt=100,
               max_iter=1000, dist='euclidean',
               margin=1.0, opt=None, rel_embedding_size=None, entity_coocurrence_matrix=None,
               smoothing_weight=0.0):
    super(TransR, self).__init__(embedding_size=embedding_size,
                                 maxnorm=1.0,
                                 batch_pos_cnt=batch_pos_cnt,
                                 max_iter=max_iter,
                                 model_type='ranking_margin',
                                 ...
                                 entity_coocurrence_matrix=entity_coocurrence_matrix,
                                     smoothing_weight=smoothing_weight)
    self.rel_embedding_size = embedding_size if rel_embedding_size is None else rel_embedding_size
    self.dist = dist
    self.margin = margin
  ...
  def _create_model(self, train_triples, initial_entity_embeddings=None):
    ...
    entity_cnt = len(set(train_triples[:, 0]).union(train_triples[:, 2]))
    rel_cnt = len(set(train_triples[:, 1]))
    print("Creating TransR model with %s entities and %s relations" % (entity_cnt, rel_cnt))
    init_sd = 1.0 / np.sqrt(self.embedding_size)
    # Embedding variables
    entity_var_shape = [entity_cnt, self.embedding_size]
    rel_var_shape = [rel_cnt, self.embedding_size]
    entity_init = tf.truncated_normal(entity_var_shape, stddev=init_sd) \
      if initial_entity_embeddings is None else initial_entity_embeddings
    rel_init = tf.truncated_normal(rel_var_shape, stddev=init_sd)
    rel_mat_init = tf.batch_matrix_diag(tf.ones([rel_cnt, self.rel_embedding_size])) if self.rel_embedding_size == self.embedding_size\
      else tf.ones((rel_cnt, self.embedding_size, self.rel_embedding_size))
    # Ensure maxnorm constraints are initially satisfied
    entity_init = dense_maxnorm(entity_init, self.maxnorm)
    self.entity_embedding_vars = tf.Variable(entity_init)
    self.rel_embedding_vars = tf.Variable(rel_init)
    self.rel_mat_vars = tf.Variable(rel_mat_init)

    # Embedding layer for each (head, rel, tail) triple being fed in as input
    head_embed = tf.nn.embedding_lookup(self.entity_embedding_vars, self.head_input)
    tail_embed = tf.nn.embedding_lookup(self.entity_embedding_vars, self.tail_input)
    rel_embed = tf.nn.embedding_lookup(self.rel_embedding_vars, self.rel_input)
    rel_mat = tf.nn.embedding_lookup(self.rel_mat_vars, self.rel_input)

    # Relationship vector acts as a translation in relation embedding space
    t = self._batch_matmul(tail_embed, rel_mat)
    h = self._batch_matmul(head_embed, rel_mat)
    diff_vec = t - (h + rel_embed)
    # negative dist so higher scores are better (important for pairwise loss)
    if self.dist == 'manhattan':
      raw_output = -tf.reduce_sum(tf.abs(diff_vec), 1)
    elif self.dist == 'euclidean':
    ...
    if self.do_smoothing:
        self.smoothing_penalty = self._relation_smoothing_penalty(self.entity_embedding_vars)
        total_loss += self.smoothing_penalty
    # Optimization with postprocessing to limit embedding vars to L2 ball
    self.train_step = self.opt.minimize(total_loss)
    unique_ent_indices = tf.unique(tf.concat(0, [self.head_input, self.tail_input]))[0]
    self.post_step = [self._norm_constraint_op(self.entity_embedding_vars,
                                               unique_ent_indices,
                                               self.maxnorm),
                      self._norm_constraint_op(self.rel_embedding_vars,
                                               None,
                                               self.maxnorm)
                      ]
    normed_mats = []
    for rel_mat_var in tf.unpack(self.rel_mat_vars):
      normed_mats.append(dense_maxnorm(rel_mat_var, self.maxnorm))
    self.post_step.append(tf.assign(self.rel_mat_vars, tf.pack(normed_mats)))

  def _batch_matmul(self, vectors, matrices):
    return tf.pack([tf.squeeze(tf.matmul(tf.reshape(v, [1, self.embedding_size]), m))
                    for (v, m) in zip(tf.unpack(vectors), tf.unpack(matrices))])
