...
/**
  ...
  */
object mk_translator_data_svm extends App {
  ...
  vocab.add("_UNK")
   ...
   for (plog <- managed(ProgressLogger.fixedSize("Processing", corpus.getIdStream.count(), 10l, TimeUnit.SECONDS))) {
     ...
     for (id <- corpus.getIdStream.iterator().asScala) {
       if (counter == 0) {
         descs = Files.newBufferedWriter(dst.resolve("train.svml"))
       } else if (counter == devStart) {
         descs.close()
         descs = Files.newBufferedWriter(dst.resolve("dev.svml"))
       } else if (counter == testStart) {
         descs.close()
         descs = Files.newBufferedWriter(dst.resolve("test.svml"))
       }
       counter += 1

       for (doc <- managed(corpus.load(id))) {
           val intrClass = if (doc.get(EegNote.interpretation) == "ABNORMAL") 1 else -1
...
//           }
           val fs = desc.map(t => vocab.getIDOrAdd(t.toString)).distinct.sorted.map(t => s"$t:1")

          if (fs.nonEmpty) {
            descs.append(s"$intrClass ${fs.mkString(" ")} # $id\n")
          } else {
            failed += 1
           }
