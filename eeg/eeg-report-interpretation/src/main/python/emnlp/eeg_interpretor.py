    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            for j in range(FLAGS.batch_size):
                ...
                guess = outputs2words(outputs, rev_impr_vocab)
                #print("Guess:", guess)
                gold = outputs2words([word[j] for word in decoder_inputs], rev_impr_vocab)
                ...
                bleus.append(bleu)
                rouges.append(rouge)
            print("    STEP", current_step, "Ex. Guess:", " ".join(guess))
            print("    STEP", current_step, "Ex. Gold:", " ".join(gold))
            ...
            if current_step % FLAGS.steps_per_checkpoint == 0:
                ...
                print("global step", model.global_step.eval(), "learning rate %.4f step-time %.2f" % (model.learning_rate.eval(), step_time))
                print("  TRAIN AVG: Perplexity = %.4f" % perplexity)
                print("  TRAIN AVG: Bleu-2 = %.4f" % gmean(bleus))
                print("  TRAIN AVG: Rouge-2 = %.4f:" % gmean(rouges))
                sys.stdout.flush()
                bleus = []
                rouges = []
            ...
            if current_step % FLAGS.steps_per_eval == 0:
                ...
                for j in range(FLAGS.num_dev_eval_samples):
                    ...
                    for k in range(FLAGS.batch_size):
                        ...
                        guess = outputs2words(outputs, rev_impr_vocab)
                        gold = outputs2words([word[k] for word in decoder_inputs], rev_impr_vocab)
                        bleus.append(get_precision_bleu(guess, gold))
                        rouges.append(get_recall_rouge(guess, gold))
                ppx /= (FLAGS.num_dev_eval_samples)
                print("  DEVEL: Guess:", " ".join(guess))
                print("  DEVEL: Gold:", " ".join(gold))
                print("  DEVEL: Average Perplexity = %.4f" % ppx)
                print("  DEVEL: Bleu-2 = %.4f", gmean(bleus))
                print("  DEVEL: Rouge-2 = %.4f:", gmean(rouges))
                sys.stdout.flush()
                bleus = []
                rouges = []
                    #devel_summary = tf.Summary()
                    #devel_perplexity_summary = devel_summary.value.add()
                    #devel_perplexity_summary.tag = "Development: AVG Perplexity"
                    #devel_perplexity_summary.simple_value = ppx
                    #devel_writer.add_summary(devel_perplexity_summary, model.global_step.eval())
...
def words2ngrams(words, n=2):
    #print("Words:", words)
    return ["_".join(tuple) for tuple in zip(*(words[i:] for i in range(n)))]

def get_precision_bleu(guess, gold, n=2):
    #print("Guess:", guess)
    guess=words2ngrams(guess)
    gold=words2ngrams(gold)
    ...
    return np.float32(sum) / len(f_guess.keys())
...
def get_recall_rouge(guess, gold, n=2):
    ...
    for ngram in f_gold.iterkeys():
        #print("Count[%s] = Gold: %d; Guess: %d" % (ngram, f_gold[ngram], f_guess[ngram]))
        sum += min(f_guess[ngram], f_gold[ngram])
    return np.float32(sum) / len(f_gold.keys())
