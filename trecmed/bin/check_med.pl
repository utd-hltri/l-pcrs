...
# Check a TREC 2012 medical records track submission for various
# common errors:
#      * extra fields
#      * multiple run tags
...
# If you have a master list of collection visit IDs, this
# script will read it and check that all your visitIds are valid.
# If the file is not present, then an approximate check is used, but
# it will only be helpful in detecting things that are obviously not
# visit IDs.  The master list of visit IDs is posted on the same page
# as this script in the active participants' section of the TREC web site.
# You are strongly encouraged to make use of this check.


# Change these variable values to the directory in which the list
# of visitID's resides and the directory where the error log should be put
my $docno_loc = "/runs/aux/med-visits.txt";
my $errlog_dir = ".";
...
while ($line = <RESULTS>) {
    ...
    if (exists $valid_ids{$docno} && $valid_ids{$docno} eq $topic) {
	&error("Visit `$docno' retrieved more than once for topic $topic");
	next;
    } elsif (check_docno($docno)) {
	$valid_ids{$docno} = $topic;
    } else {
	&error("Unknown visit `$docno'");
	next;
    }
    $numret{$topic}++;
}
