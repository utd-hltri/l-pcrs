...
/**
 * Created by travis on 1/24/15.
 */
class PMCParagraphIndexer (
  ...
  private val xParagraph = Local(xPath.compile("/article/body//p", Filters.element()))
  ...
  private val pCount = new AtomicInteger(0)
   ...
   */
  def texts(xpath: XPathExpression[Element])(implicit xml: XMLDocument): Seq[String] = {
    for (text: Element <- xpath.evaluate(xml).asScala) yield text.getValue
  }

  def text(xpath: XPathExpression[Element])(implicit xml: XMLDocument): String = {
    xpath.evaluateFirst(xml).getValue
  }
  ...
  private def addMergedField(name: String, xpath: XPathExpression[Element], fieldType: FieldType, required: Boolean = true)
    ...
    def indexFile(file: File): Unit = {
      ...
      try {
        val pmcid = text(xPMC.get)

        for ((p, n) <- texts(xParagraph.get).zipWithIndex) {
          implicit val document = new Document()
          document.add(new Field("pmc_id", pmcid, StringField.TYPE_STORED))
          document.add(new Field("pid", n.toString, StringField.TYPE_STORED))
          document.add(new Field("uid", pmcid + "#" + n, StringField.TYPE_STORED))
          document.add(new Field("text", p, textField))

          writer.addDocument(document)
          log.debug(
            f"Indexed paragraph ${pCount.incrementAndGet()}%,d ($pmcid%s#$n%d) in ${
              (System.nanoTime() - start).toDouble / 1e9
            }%5.3f s"
          )
        }
        log.debug(
          f"Indexed document ${count.incrementAndGet()}%,d (${file.getName}%s) in ${
            (System.nanoTime() - start).toDouble / 1e9
          }%5.3f s"
        )
    ...
    override def close(): Unit = {
      synchronized {
        ...
        writer.close()
        log.info(
          f"Finished indexing ${count.get()}%,d documents and ${pCount.get()}%,d paragraphs in ${
   ...
   */
  object PMCParagraphIndexer extends App with Logging {
    ...
    using(new PMCParagraphIndexer(index)) { indexer =>
